...
v_saldo,state,origin,usage,complete_name,ubicacion,inventory_id ,picking_id  
from (
...
--------------- EMPIEZA LA UNION 
    ...
    costo_unit
		, v_entrada,v_salida,v_saldo,state,origin,
		...
		(
			...
			,(sm.price_unit * sm.product_qty) as v_entrada,(sm.product_qty * 0)v_salida,(sm.product_qty *0)v_saldo,sm.state,sm.origin,
		...
		from
		   (
				select sm.id,sm.date_expected,sm.product_id,sm.name,sm.company_id,(sm.product_qty * 0) as u_entrada,
                                 sm.product_qty as u_salida,(sm.price_unit) as costo_unit
                                 ,(sm.price_unit*sm.product_qty*0)as v_entrada,(sm.price_unit * sm.product_qty )v_salida,(sm.price_unit *0)v_saldo,
                                  sm.state,sm.origin
